...
class wccpf_field_textarea extends wccpf_product_field {
	...
	function render_product_field( $field ) { 
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<textarea class="wccpf-field" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" rows="<?php echo esc_attr( $field["rows"] ); ?>" maxlength="<?php echo esc_attr( $field["maxlength"] ); ?>"><?php echo esc_html( $field["default_value"] ); ?></textarea>
